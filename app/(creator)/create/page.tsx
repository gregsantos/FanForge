    ...
    const fetchCampaignAssets = async () => {
      try {
        ...
        if (!campaignId) {
          ...
          console.log('No campaign ID provided, using mock data')
          setCampaign({
            id: 'mock',
            title: 'Demo Campaign - No Campaign Selected',
            description: 'This is demo mode. Select a campaign from the discover page.',
            brandId: 'mock',
        ...
        if (campaignData.assets && Array.isArray(campaignData.assets)) {
          console.log('Using assets from campaign response:', campaignData.assets)
        ...
        // Otherwise fetch assets for the campaign's IP kit (real data format)
          ...
          const assetsResponse = await fetch(`/api/assets?ipKitId=${campaignData.ipKitId}`)
          ...
          console.log('No IP kit ID found in campaign, using mock assets as fallback')
          setAssets(mockAssets)
        }
      } catch (err) {
